...
export class AddressController {
    constructor(private addressUseCase: AddressUseCase, private socketAdapter: SocketAdapter) {
        ...
        this.getAllWithClientCtrl = this.getAllWithClientCtrl.bind(this);
    }
    ...
    public async getAllWithClientCtrl(req: Request, res: Response) {
        try {
            const cmp_uuid = req.params.cmp_uuid;
            ...
            if (page && perPage) {
                const address = await this.addressUseCase.getAddressesWithClient(cmp_uuid, rou_uuid)
                return res.status(200).send({
                    success: true,
                    message: 'Addresses with client retornados.',
                    ...paginator(address, page.toString(), perPage.toString())
                });
            } else {
                const address = await this.addressUseCase.getAddressesWithClient(cmp_uuid, rou_uuid)
            ...
            console.error('Error en getAllWithClientCtrl (controller):', error.message);
            return res.status(400).json({
                success: false,
                message: 'No se pudo recuperar los addresses with client.',
                error: error.message, // Mensaje claro del error
            });
        }
    }
